...
repeat(Dom, FromTo, FromTo, Res) :-
  repeat(Dom, FromTo, Res).
...
concatenation(A1, A2, automaton_dom(States3,Delta3,Start1,End2Star)) :-
  ...
  findall((S,epsilon,T), (   member(S, End1),
                             member(T, Start2Star)
                         ), Trans),
